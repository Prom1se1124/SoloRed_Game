...
/**
 * Test Cases for basic SoloRed Game Model.
 */
public class SoloRedGameModelTest {
  ...
  public void testDrawForHand() {
    game1.startGame(deck1,false,4,4);
    game1.playToPalette(0,0);
    ConcreteCard firstCard = game1.getHand().get(0);
    game1.drawForHand();
    // check if it fills the hand card
    Assert.assertEquals(game1.getHand().size(), 4);
    // check if the hand card sequence is messed up
    Assert.assertEquals(game1.getHand().get(0), firstCard);
  }
